...
- ASA sleeve:
  ...
  - Vent behavior uses a large separate tripod-side panel cutout:
    ...
    - Maki shade connector bars run only the body depth (`0.0` to `133.92 mm`) and do not extend into the front/rear shade overhang zones; they are directly fused into the final ASA sleeve body after shade union so they cannot disappear as loose shade sub-geometry,
  ...
  - Sun shade cover extends `6.35 mm` (`0.25 in`) past both the front and rear shell ends; support ribs run only over the body depth and not into either overhang, and each shade end has a cut-back lofted `1.5 mm` outward shade-perimeter extension that shrinks `1.5 mm` inward toward the case while using the same rounded side-skirt lower trim as the shade cover and only a `0.15 mm` fuse overlap at the shade end.
