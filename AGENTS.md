...
- Geometry intent:
  ...
  - External sun shade extends `6.35 mm` behind the rear body face to overhang the back/sides; support ribs stay within the original body depth, the rear end has a sloped `3.0 mm` out × `3.0 mm` down drip lip, and outside-facing shade end edges are rounded with the largest stable fillet (`1.0 mm` in the current generated ASA shell).
