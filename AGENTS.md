...
- ASA sleeve:
  ...
  - Sun shade cover extends `6.35 mm` (`0.25 in`) past both the front and rear shell ends; support ribs run the full shade depth, each end has a lofted `3.0 mm` outward shade-perimeter extension that shrinks `3.0 mm` inward toward the case, and outside-facing front/rear shade end edges are rounded with the largest stable fillet (`1.0 mm` in the current generated ASA shell).
